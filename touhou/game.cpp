...
void initBoos1() {
	...
	//玩家
	player = new Player(LIFE, -2, -4.09, playerTexture);
	...
	enemy = new Enemy(100, -2, 2, enemyTexture);
	enemy->addMovePosition(0,0,15);
	enemy->addMovePosition(0, 1, 10);

	RenderObject* boosVAO = enemy->getRedObj();
...
void initShader() {
	...
	lineShader = new Shader(".\\assets\\shaders\\vertex.glsl",".\\assets\\shaders\\fragmentLine.glsl");
}
//初始化游戏UI 
void intiShow() {
	for (int i = 0; i < 10;i++) {
...
void gameObjectEvent() {
	...
	for (int i = 0; i < enemyBullet.size(); i++) {
		...
		if (game!=nullptr) {
			...
			if (player->getOP()==false && compleDistance((game->getWidth() / 2) * STD_PIX_SIZE, player->getCenter_x(),
				player->getCenter_y(), game->getCenter_x(), game->getCenter_y())) {
				if (life_update.load()) {
					if (player->getLife()>0) {
...
void playerEvent(GLFWwindow* window) {
	...
	float new_x = player->getCenter_x();
	float new_y = player->getCenter_y();
	...
	float playerHalfWidth = (player->getWidth() / 2) * STD_PIX_SIZE;
	float playerHalfHeight = (player->getHeight() / 2) * STD_PIX_SIZE;

	float playerCenterX = player->getCenter_x();
	float playerCenterY = player->getCenter_y();
	...
	if (playerCenterX >= left + playerHalfWidth && playerCenterX <= right - playerHalfWidth &&
		...
		if (playerCenterX - playerHalfWidth < left) {
			player->updatePosition(left + playerHalfWidth, playerCenterY); // 移动到左边界内
		}
		else if (playerCenterX + playerHalfWidth > right) {
			player->updatePosition(right - playerHalfWidth, playerCenterY); // 移动到右边界内
		}

		if (playerCenterY - playerHalfHeight < bottom) {
			player->updatePosition(playerCenterX, bottom + playerHalfHeight); // 移动到底边界内
		}
		else if (playerCenterY + playerHalfHeight > top) {
			player->updatePosition(playerCenterX, top - playerHalfHeight); // 移动到顶边界内
		}
	}
	if (glfwGetKey(window, GLFW_KEY_Z) == GLFW_PRESS || glfwGetKey(window, GLFW_KEY_I) == GLFW_PRESS) {
		if (gameTick_update&& playerBullet.size() <= MaxPlayerBullet) {
			gameTick_update = (false);
			gameTick->addBool(&gameTick_update, 2);
...
void debug(int key) {
	if (player != nullptr) {
		if (key == GLFW_KEY_F3) {
			if (player->getOP() == true) {
				std::cout << "玩家处于自机状态" << std::endl;
				player->setOP(false);
			}
			else {
				std::cout << "玩家处于操作员状态" << std::endl;
				player->setOP(true);
			}
		}
	}
	if (key== GLFW_KEY_F2) {
		renderClass = (renderClass + 1) % (2 + 1);
		if(renderClass==0) {
			std::cout << "标准渲染" << std::endl;
		}
		if (renderClass == 1) {
			std::cout << "线框+标准渲染" << std::endl;
		}
		if(renderClass == 2){
			std::cout << "线框渲染" << std::endl;
		}
	}
}
...
void gameActivite() {
	if (activite == GAME_STOP|| activite == PLAYER_DEAD) {
		...
		rend(blenderShader, player->getRenderObject(), playerTexture);
...
void rend(Shader* shader,RenderObject *vao,Texture *mTexture) {
	...
	if (renderClass == 1) {
		...
		lineShader->IntUniform("samplerTexture", mTexture->getUnit());
		lineShader->setMatrix4x4("transform", transform);
		lineShader->setMatrix4x4("viewMatrix", viewMatrix);
		lineShader->setMatrix4x4("projectionMatrix", orthoMatrix);
		mTexture->bind();
...
void createPlayerBullet() {
		RenderObject* vao = new RenderObject(new GameObject(player->getCenter_x(), player->getCenter_y(), playerTexture->getWidth(), playerTexture->getHeight(), BULLET));
